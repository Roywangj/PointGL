...
def main(gpu, cfg, profile=False):
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if epoch > 5:
            train_time_total = train_time_total + (train_time_end - train_time_start)
            logging.info(f'epoch:{epoch}, training time: {train_time_total}, epochs: {epoch-5}, ave time:{train_time_total/(epoch-5)}')
        ...
        if epoch > 5:
            test_time_total = test_time_total + (test_time_end - test_time_start)
            logging.info(f'epoch:{epoch}, testing time: {test_time_total}, epochs: {epoch-5}, ave time:{test_time_total/(epoch-5)}')
